...
class TritonPythonModel:
    ...
    def execute(self, requests):
        ...
        for request in requests:
            # Get INPUT0
            inp = pb_utils.get_input_tensor_by_name(request, "deoldify_preprocessing_input")
            img = inp.as_numpy()

            img_np = cv2.resize(img, (256, 256), interpolation=cv2.INTER_CUBIC)
            img_np = np.asarray(img_np, dtype=np.float32)
            img_np = np.transpose(img_np, axes=(2, 0, 1))

            out_tensor = pb_utils.Tensor("deoldify_preprocessing_output", img_np.astype(self.output_type))
